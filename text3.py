...
def get_good(name):
    try:
        ...
        '''while(True):
            a = driver.find_element_by_xpath('// *[ @ id = "dispage"] / a[3]') 
            a.click()'''
        goodList = []
        ...
        for good in good_list:
            ...
            for char in range(len(good_url)):
                if(good_url[char] == '='):
                    LeftFlag = char
                if(good_url[char] == '&'):
                    ReftFlag = char
                    good_url = good_url[LeftFlag+1:ReftFlag]
                    break
            ...
            good_pic = good.find_element_by_css_selector(
                '.shenqingGY img').get_attribute('src')
            # 评价人数
